...
@pytest.mark.asyncio
async def test_non_dry_scheduler_plan_requires_exact_approval_before_mcp(tmp_path):
    settings = Settings(allow_order_execution=True, dry_run=False)
    journal = DecisionJournal(tmp_path / "journal.jsonl")
    trade_plan = plan().model_copy(
        update={
            "approval_expires_at": datetime.now(UTC) + timedelta(minutes=5),
    ...
    mcp = CaptureMCP()
    result = await PaperExecutionAgent(settings, journal, mcp).submit(trade_plan, gate)
    assert result["status"] == "approval_required"
    assert result["plan_id"] == trade_plan.plan_id
    assert mcp.calls == []
    assert journal.approved_plan(trade_plan.plan_id).plan_id == trade_plan.plan_id


@pytest.mark.asyncio
async def test_reviewed_plan_cannot_change_before_exact_submission(tmp_path):
    dry_settings = Settings(allow_order_execution=True, dry_run=True)
    journal = DecisionJournal(tmp_path / "journal.jsonl")
    trade_plan = plan().model_copy(
        update={
    ...
    gate = DeterministicRiskGate(dry_settings).assess(
        trade_plan, market_open=True, open_positions=0, buying_power=10_000
    )
    await PaperExecutionAgent(dry_settings, journal, NoCallMCP()).submit(trade_plan, gate)
    changed = trade_plan.model_copy(
        update={"thesis": trade_plan.thesis.model_copy(update={"confidence": 0.1})}
    )

    result = await PaperExecutionAgent(
        Settings(allow_order_execution=True, dry_run=False), journal, NoCallMCP()
    ).submit_approved(changed, gate)

    assert result == {"status": "blocked", "reasons": ["approved plan changed after review"]}
...
async def test_guardian_exit_reverses_legs_atomically_and_stays_dry_run(tmp_path):
    ...
    journal.append(
        JournalEntry(
            event="order_submission_intent",
            plan=entry_plan,
            gate=GateResult(approved=True, reasons=[]),
        )
    )
    exit_plan = entry_plan.closing_plan(executable_credit=1.8)
...
def test_mcp_wrapped_order_receipt_is_parsed_for_acknowledgement():
    receipt = {
        "tool": "place_option_order",
        "result": ['{"id":"paper-order-2","status":"accepted"}'],
    }
    assert PaperExecutionAgent._provider_status(receipt) == "accepted"
    assert PaperExecutionAgent._provider_order_id(receipt) == "paper-order-2"

...
def dashboard_state(journal: DecisionJournal, *, limit: int = 20) -> dict[str, Any]:
    ...
    position_guardian = journal.guardian_status()
    trade_update_monitor = journal.monitor_status()
    ...
    completed_client_order_ids = {
        str(trade["client_order_id"])
    ...
    for event in reversed(events):
        ...
        if (
            client_order_id
            and str(client_order_id) not in completed_client_order_ids
            and value is not None
        ):
            latest_marks[str(client_order_id)] = float(value)
    realized_before_fees = round(
        sum(float(trade.get("realized_pnl_before_fees") or 0) for trade in completed_paper_trades),
        2,
    )
    realized_after_fees = (
        round(
            sum(float(trade["realized_pnl_after_fees"]) for trade in completed_paper_trades),
            2,
        )
        if completed_paper_trades
        and all(
            trade.get("realized_pnl_after_fees") is not None for trade in completed_paper_trades
        )
        else None
    )
    return {
        ...
        "trade_update_monitor": trade_update_monitor,
        "summary": {
            ...
            "realized_paper_pnl_before_fees": realized_before_fees,
            "realized_paper_pnl_after_fees": realized_after_fees,
            "realized_paper_pnl_display": (
                realized_after_fees if realized_after_fees is not None else realized_before_fees
            ),
            "realized_paper_pnl_basis": (
                "after_reported_fees"
                if realized_after_fees is not None
                else "gross_before_unreported_fees"
                if completed_paper_trades
...
<section class="card"><div class="section-head"><h2>Agent status</h2><span id="agent-status-badge" class="status-badge stopped">STOPPED</span></div><div class="agent-facts"><div class="fact"><strong>Entry scheduler</strong><span id="scheduler-state">—</span></div><div class="fact"><strong>Scheduler owner</strong><span id="scheduler-owner">—</span></div><div class="fact"><strong>Position guardian</strong><span id="guardian-state">—</span></div><div class="fact"><strong>Recovered spreads</strong><span id="recovered-spreads">—</span></div><div class="fact"><strong>Matched legs</strong><span id="matched-legs">—</span></div><div class="fact"><strong>Last reconciliation</strong><span id="last-reconciliation">—</span></div><div class="fact"><strong>Trade monitor</strong><span id="monitor-state">—</span></div><div class="fact"><strong>Last cycle</strong><span id="last-cycle">—</span></div><div class="fact"><strong>Next cycle</strong><span id="next-cycle">—</span></div><div class="fact"><strong>Market</strong><span id="market-state">—</span></div><div class="fact"><strong>Last error</strong><span id="last-error">None</span></div></div><pre id="agent">Loading…</pre></section>
...
const refreshDashboard=()=>fetch('/dashboard/state').then(response=>response.json()).then(d=>{{const summary=d.summary;const scheduler=d.agent.scheduler||d.scheduler||{{}};const guardian=d.agent.position_guardian||d.position_guardian||{{}};const monitor=d.agent.trade_update_monitor||d.trade_update_monitor||{{}};const paper=d.agent.paper_execution;const schedulerStatus=String(scheduler.status||'unknown');const pnlBasis=summary.realized_paper_pnl_basis==='gross_before_unreported_fees'?'gross before fees · fees not reported':summary.realized_paper_pnl_basis==='after_reported_fees'?'after reported fees':'no completed trades';const cards=[['SIMULATION',String(d.agent.simulation.status||'idle').toUpperCase(),d.agent.simulation.mode||'fixture replay only'],['HYPOTHETICAL SHADOW RESULTS',summary.hypothetical_reprice_count+' reprices',displayMoney(summary.selected_hypothetical_pnl)+' selected evidence'],['APPROVED PLANS',summary.approved_production_plan_count+summary.approved_exploration_plan_count,'production + exploration'],['ACKNOWLEDGED ORDERS',summary.acknowledged_paper_order_count,'Alpaca acknowledgement only'],['FILLS',summary.filled_paper_trade_count,'paper entry fills only'],['REALIZED PAPER P&L',displayMoney(summary.realized_paper_pnl_display),summary.realized_paper_trade_count+' completed · '+pnlBasis]];document.querySelector('#cards').innerHTML=cards.map(item=>`<div class="card metric"><div class="label">${{escapeHtml(item[0])}}</div><div class="n">${{escapeHtml(item[1])}}</div><div class="hint">${{escapeHtml(item[2])}}</div></div>`).join('');document.querySelector('#fill-badge').textContent=summary.realized_paper_trade_count?summary.realized_paper_trade_count+' COMPLETE PAPER TRADE':summary.filled_paper_trade_count?summary.filled_paper_trade_count+' PAPER ENTRY FILLS':'NO FILLS YET';document.querySelector('#scheduler-state').textContent=schedulerStatus.toUpperCase();document.querySelector('#scheduler-owner').textContent=String(scheduler.source||'durable_history').replaceAll('_',' ').toUpperCase();document.querySelector('#guardian-state').textContent=String(guardian.status||'never_started').toUpperCase();document.querySelector('#recovered-spreads').textContent=guardian.recovered_spread_count??guardian.managed_position_count??0;document.querySelector('#matched-legs').textContent=guardian.matched_leg_count??0;document.querySelector('#last-reconciliation').textContent=displayTime(guardian.last_reconciliation_at);document.querySelector('#monitor-state').textContent=String(monitor.status||'never_started').toUpperCase();document.querySelector('#last-cycle').textContent=displayTime(scheduler.last_cycle_completed_at);document.querySelector('#next-cycle').textContent=displayTime(scheduler.next_run_at);document.querySelector('#market-state').textContent=scheduler.market_open===true?'OPEN':scheduler.market_open===false?'CLOSED':'UNKNOWN';document.querySelector('#last-error').textContent=scheduler.last_error||guardian.last_error||monitor.last_error||'None';const statusNode=document.querySelector('#agent-status-badge');statusNode.textContent=schedulerStatus.toUpperCase()+(scheduler.source==='external_cli'?' · EXTERNAL CLI':'');statusNode.className='status-badge '+schedulerStatus;document.querySelector('#candidates').textContent=JSON.stringify(d.shadow_candidates,null,2);document.querySelector('#risk-budget').textContent=JSON.stringify(d.risk_budget_rejections,null,2);document.querySelector('#reprices').textContent=JSON.stringify(d.shadow_reprices,null,2);document.querySelector('#shadows').textContent=JSON.stringify(d.shadows,null,2);document.querySelector('#scheduler').textContent=JSON.stringify(scheduler,null,2);document.querySelector('#journal').textContent=JSON.stringify(d.journal,null,2);document.querySelector('#agent').textContent=JSON.stringify(d.agent,null,2);document.querySelector('#thesis-explanation').textContent=JSON.stringify(d.latest_thesis_explanation||{{}},null,2);document.querySelector('#paper-lock').textContent=paper.emergency_stop_active?'Paper Execution: EMERGENCY STOP ACTIVE':paper.locked?'Paper Execution: LOCKED by default — configuration and session arm required':'Paper Execution: ARMED for one exact unexpired plan attempt';document.querySelector('#submit-plan').disabled=paper.locked;renderTimeline(d.journal);}}).catch(()=>document.querySelector('#agent').textContent='Dashboard read unavailable');
...
function render(d){
  ...
  const report=d.shadow_session_report||{};
  const threshold=(report.hypothetical_pnl_by_threshold||{})['40']||{};
  const status=String(sch.status||'unknown');
  const market=sch.market_open===true?'OPEN':sch.market_open===false?'CLOSED':'UNKNOWN';
  const complete=(s.realized_paper_trade_count||0)>0;
  const realized=s.realized_paper_pnl_display;
  const basis=s.realized_paper_pnl_basis==='gross_before_unreported_fees'?'GROSS BEFORE FEES · FEES NOT REPORTED':s.realized_paper_pnl_basis==='after_reported_fees'?'AFTER REPORTED FEES':'PROVIDER EXITS ONLY';
  document.querySelector('#agent-label').textContent=status+(market==='CLOSED'?' · market gated':'');
  document.querySelector('#market').textContent=market;
  document.querySelector('#last-cycle').textContent=clock(sch.last_cycle_completed_at);
  document.querySelector('#next-cycle').textContent=clock(sch.next_run_at);
  document.querySelector('#events').textContent=d.event_count??0;
  document.querySelector('#opportunities').textContent=report.opportunity_count??s.shadow_opportunity_count??0;
  document.querySelector('#reprices').textContent=report.reprice_count??s.hypothetical_reprice_count??0;
  document.querySelector('#fills').textContent=s.filled_paper_trade_count??0;
  document.querySelector('#open-pnl').textContent=money(s.open_position_unrealized_pnl);
  document.querySelector('#realized-pnl').textContent=money(realized);
  document.querySelector('#realized-basis').textContent=basis;
  document.querySelector('#shadow-pnl').textContent=money(threshold.net_hypothetical_pnl);
  document.querySelector('#qualification').textContent=report.qualification_rate===undefined?'—':Math.round(Number(report.qualification_rate)*100)+'%';
  const lifecycleBadge=document.querySelector('#lifecycle-badge');
  lifecycleBadge.textContent=complete?'LIFECYCLE COMPLETE':'EXIT PENDING';
  lifecycleBadge.className=complete?'badge exit closed':'badge exit';
  const exitStep=document.querySelector('#exit-step');
  exitStep.className=complete?'step done':'step current';
  exitStep.querySelector('span').textContent=complete?'EXIT FILLED':'EXIT PENDING';
  document.querySelector('#realized-step').className=complete?'step done':'step';
  const events=(d.journal||[]).slice(0,8);
  document.querySelector('#timeline').innerHTML=events.map(e=>`<li><i></i><strong>${label(e.event)}</strong><time>${clock(e.timestamp)}</time></li>`).join('')||'<li><i></i><strong>No events</strong><time>—</time></li>';
}

...
:root{color-scheme:dark;--bg:#050916;--panel:#0d1730;--line:#22375d;--text:#f3f7ff;--muted:#91a5c7;--cyan:#4bdcff;--green:#5bf0a5;--amber:#ffd166;--red:#ff7185}
*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 82% 5%,#163b67 0,transparent 34%),radial-gradient(circle at 12% 88%,#123d39 0,transparent 30%),var(--bg);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,sans-serif;padding:34px}.frame{max-width:1480px;margin:auto}.top{display:flex;justify-content:space-between;align-items:flex-start;gap:24px}.eyebrow{font-size:12px;font-weight:900;letter-spacing:.22em;color:var(--cyan)}h1{font-size:clamp(42px,6vw,82px);line-height:.92;letter-spacing:-.065em;margin:14px 0 16px;max-width:900px}.lede{max-width:760px;color:#bac8df;font-size:17px;line-height:1.55}.badges{display:flex;gap:10px;flex-wrap:wrap;justify-content:flex-end}.badge{border:1px solid var(--line);border-radius:999px;padding:9px 13px;font-size:11px;font-weight:900;letter-spacing:.11em;white-space:nowrap}.paper{color:var(--green);border-color:#27755a;background:#0c3b2c}.exit{color:var(--amber);border-color:#765f26;background:#372d13}.live{color:var(--cyan);border-color:#2d7190;background:#0d3040}.pulse{width:8px;height:8px;border-radius:50%;background:currentColor;display:inline-block;margin-right:8px;box-shadow:0 0 0 0 currentColor;animation:pulse 1.8s infinite}@keyframes pulse{70%{box-shadow:0 0 0 9px transparent}}
.status{margin:26px 0 16px;background:linear-gradient(120deg,#102343,#0a172c);border:1px solid #2d5483;border-radius:22px;padding:20px 22px;display:grid;grid-template-columns:1.4fr repeat(3,1fr);gap:14px;align-items:center}.status-main{display:flex;align-items:center;gap:14px}.orb{width:48px;height:48px;border-radius:50%;background:#113d36;border:1px solid #2f916c;display:grid;place-items:center;color:var(--green);font-size:19px;box-shadow:0 0 24px #5bf0a533}.status-main strong{display:block;font-size:18px}.status-main span,.fact span{display:block;color:var(--muted);font-size:12px;margin-top:4px}.fact{border-left:1px solid var(--line);padding-left:16px}.fact strong{font-size:13px;letter-spacing:.04em}
.metrics{display:grid;grid-template-columns:repeat(6,1fr);gap:12px;margin:16px 0}.metric,.panel{background:linear-gradient(145deg,#101c37,#0a1327);border:1px solid var(--line);border-radius:18px;padding:18px}.metric label{display:block;color:var(--muted);font-size:10px;font-weight:900;letter-spacing:.12em;text-transform:uppercase}.metric b{display:block;font-size:28px;margin:11px 0 5px}.metric small{color:var(--muted);font-size:11px}.metric.positive{border-color:#276b55}.metric.attention{border-color:#745b27}.metric.negative{border-color:#7c3342}.negative b{color:var(--red)}
.lower{display:grid;grid-template-columns:1.1fr .9fr;gap:16px}.panel-head{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:15px}.panel h2{font-size:15px;margin:0}.panel-tag{font-size:10px;color:var(--muted);font-weight:900;letter-spacing:.1em}.steps{display:grid;grid-template-columns:repeat(7,1fr);gap:7px}.step{border:1px solid var(--line);border-radius:12px;padding:12px 9px;min-height:78px}.step i{display:block;width:9px;height:9px;border-radius:50%;background:#405271;margin-bottom:9px}.step span{font-size:10px;color:var(--muted);line-height:1.3}.step.done{border-color:#2a755c;background:#0d2c26}.step.done i{background:var(--green);box-shadow:0 0 12px #5bf0a5}.step.current{border-color:#7e6428;background:#332a14}.step.current i{background:var(--amber);box-shadow:0 0 12px #ffd166}.timeline{list-style:none;padding:0;margin:0;display:grid;gap:9px}.timeline li{display:grid;grid-template-columns:12px 1fr auto;gap:10px;align-items:center;border-bottom:1px solid #1b2945;padding:0 0 9px}.timeline i{width:8px;height:8px;border-radius:50%;background:#3d73aa}.timeline li:first-child i{background:var(--green);box-shadow:0 0 12px #5bf0a5}.timeline strong{font-size:12px}.timeline time{font-size:10px;color:var(--muted)}.research{display:flex;align-items:center;justify-content:space-between;gap:12px;background:#0b1730;border:1px solid var(--line);border-radius:14px;padding:13px;margin-top:14px}.research b{font-size:22px}.research span{font-size:10px;color:var(--muted);display:block;margin-top:3px}.footer{display:flex;justify-content:space-between;gap:20px;margin-top:18px;color:var(--muted);font-size:10px;letter-spacing:.04em}.footer strong{color:var(--text)}
@media(max-width:980px){body{padding:18px}.top{display:block}.badges{justify-content:flex-start}.status{grid-template-columns:1fr 1fr}.metrics{grid-template-columns:repeat(2,1fr)}.lower{grid-template-columns:1fr}.steps{grid-template-columns:repeat(4,1fr)}}
</style></head><body><main class="frame">
<header class="top"><div><div class="eyebrow">VEGAGUARD // LIVE PROOF OF WORK</div><h1>The agent is working.<br>Risk stays in control.</h1><p class="lede">A live, read-only window into autonomous scanning, shadow evaluation and an Alpaca paper spread under deterministic position management.</p></div><div class="badges"><span class="badge paper">PAPER ONLY</span><span class="badge exit" id="lifecycle-badge">EXIT PENDING</span><span class="badge live"><i class="pulse"></i>LIVE JOURNAL</span></div></header>
<section class="status"><div class="status-main"><div class="orb"><i class="pulse"></i></div><div><strong id="agent-label">LOADING AGENT</strong><span id="agent-detail">Reading durable heartbeat…</span></div></div><div class="fact"><strong>MARKET</strong><span id="market">—</span></div><div class="fact"><strong>LAST CYCLE</strong><span id="last-cycle">—</span></div><div class="fact"><strong>NEXT CYCLE</strong><span id="next-cycle">—</span></div></section>
...
<section class="lower"><article class="panel"><div class="panel-head"><h2>Paper lifecycle</h2><span class="panel-tag">ONE DEFINED-RISK SPREAD</span></div><div class="steps"><div class="step done"><i></i><span>APPROVED</span></div><div class="step done"><i></i><span>SUBMITTED</span></div><div class="step done"><i></i><span>ACKNOWLEDGED</span></div><div class="step done"><i></i><span>FILLED</span></div><div class="step done"><i></i><span>MONITORED</span></div><div class="step current"><i></i><span>EXIT PENDING</span></div><div class="step"><i></i><span>REALIZED P&amp;L</span></div></div><div class="research"><div><b id="threshold">70</b><span>PRODUCTION THRESHOLD · UNCHANGED</span></div><div><b id="shadow-pnl">—</b><span>THRESHOLD-40 HYPOTHETICAL EVIDENCE</span></div><div><b id="qualification">—</b><span>SESSION QUALIFICATION RATE</span></div></div></article><article class="panel"><div class="panel-head"><h2>Live agent timeline</h2><span class="panel-tag">SANITIZED EVENTS</span></div><ol class="timeline" id="timeline"><li><i></i><strong>Loading journal…</strong><time>—</time></li></ol></article></section>
<footer class="footer"><span><strong>Simulation, shadow and paper evidence remain separate.</strong> No real-money path.</span><span>IDs, credentials and operator controls are intentionally hidden.</span></footer>
</main><script>
...
function render(d){const s=d.summary||{};const sch=d.scheduler||{};const report=d.shadow_session_report||{};const threshold=(report.hypothetical_pnl_by_threshold||{})['40']||{};const status=String(sch.status||'unknown');const market=sch.market_open===true?'OPEN':sch.market_open===false?'CLOSED':'UNKNOWN';document.querySelector('#agent-label').textContent=status==='waiting'?(market==='OPEN'?'AGENT WAITING / ACTIVE':'AGENT ACTIVE · SAFE WAIT'):status==='running'?'AGENT SCANNING NOW':status.toUpperCase();document.querySelector('#agent-detail').textContent=market==='CLOSED'?'Monitoring persists; entries and exits remain market-gated.':'Fresh scans and guardian checks are running.';document.querySelector('#market').textContent=market;document.querySelector('#last-cycle').textContent=clock(sch.last_cycle_completed_at);document.querySelector('#next-cycle').textContent=clock(sch.next_run_at);document.querySelector('#events').textContent=d.event_count??0;document.querySelector('#opportunities').textContent=report.opportunity_count??s.shadow_opportunity_count??0;document.querySelector('#reprices').textContent=report.reprice_count??s.hypothetical_reprice_count??0;document.querySelector('#fills').textContent=s.filled_paper_trade_count??0;document.querySelector('#open-pnl').textContent=money(s.open_position_unrealized_pnl);document.querySelector('#realized-pnl').textContent=money(s.realized_paper_pnl_after_fees);document.querySelector('#shadow-pnl').textContent=money(threshold.net_hypothetical_pnl);document.querySelector('#qualification').textContent=report.qualification_rate===undefined?'—':Math.round(Number(report.qualification_rate)*100)+'%';const complete=(s.realized_paper_trade_count||0)>0;document.querySelector('#lifecycle-badge').textContent=complete?'LIFECYCLE COMPLETE':'EXIT PENDING';const events=(d.journal||[]).slice(0,7);document.querySelector('#timeline').innerHTML=events.map(e=>`<li><i></i><strong>${label(e.event)}</strong><time>${clock(e.timestamp)}</time></li>`).join('')||'<li><i></i><strong>No events yet</strong><time>—</time></li>';}
function refresh(){fetch('/dashboard/state?limit=24').then(r=>r.json()).then(render).catch(()=>{document.querySelector('#agent-label').textContent='READ MODEL UNAVAILABLE';});}
